...
| **Pending decision** | Fail-closed retry queue under `{collector}:{fingerprint}` when a collector errors this tick. Waking observations are delivered immediately and are not queued. |
...
├── tick_state.py        # STATE_VERSION, delivered prune, pending coerce
...
- **Pending** is the fail-closed retry queue when a collector errors. Waking observations are delivered in the packed wake and are not queued.
