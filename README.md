...
`doctor` validates heartbeat JSON, imports enabled collectors, checks shims, looks up Cron jobs, and reads `state.health`: a collector blind for at least `collector_watchdog.after_ticks` ticks is an **error**, a shorter streak a warning, and one blind tick is already an error when the watchdog is disabled, because nothing else would report it. This is the difference between "is this wired correctly" and "is anything still watching" — a heartbeat can pass every config check while a collector has seen nothing for hours. `status` lists the same streaks, worst first. Neither proves the scheduler is currently firing — after `setup`, run one manual `tick --name …` and confirm either a wake JSON object or exactly `{"wakeAgent": false}` with exit code 0. Collector load/execution failures exit non-zero (Cron failure delivery), never a quiet success.
